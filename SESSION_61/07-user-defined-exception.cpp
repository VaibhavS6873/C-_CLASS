...
class verbose_runtime_error : public std::runtime_error
{
    private:
        ...
        int src_line_number;
    public:
        verbose_runtime_error(char* err_msg, char* file_name=NULL, int line_nr=-1): std::runtime_error(err_msg), src_file_name(file_name), src_line_number(line_nr){}
...
void f1(int n)
{
    if(n < 0)
    {
        throw verbose_runtime_error("Negative number ka dials?", __FILE__,__LINE__);
    }
